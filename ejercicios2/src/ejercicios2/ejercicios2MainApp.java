...
public class ejercicios2MainApp {
	...
	public static void createTableEmp(String db, String tablename) {
		try {
			...
			String Query = "CREATE TABLE "+ tablename+
					" (Dni VARCHAR(10) PRIMARY KEY, Nombre VARCHAR(50), Apellidos VARCHAR(50), Departamento int, FOREIGN KEY(Departamento) REFERENCES departamentos(Codigo) ON DELETE CASCADE ON UPDATE CASCADE);";
			Statement st= conexion.createStatement();
			st.executeUpdate(Query);
			System.out.println("la tabla"+tablename+" se ha creado con exito");
